...
Demo 已跑通：本项目可以在本机受控 Chrome 中驱动 ChatGPT、Gemini、DeepSeek 网页端进行多轮会议式对话，全程不使用官方 API。用户可以通过会议室页面上传资料、设置多个议程，并让三个网页端 AI 按接力或并行模式讨论，最后生成本地会议纪要。

下一阶段文件链路设计：优先把用户上传的原始文件直接分发给各家网页端 AI；每家上传失败时重试 1-2 次；仍失败则使用后端抽取出的文本资料包作为兜底。扫描版 PDF/OCR 暂不作为首批目标。
