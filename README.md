...
```bash
npm run pack    # 生成干净的 ai-meetingroom.zip（约 1MB，仅源码）
```

**发给别人请用这个 zip**，不要直接压缩整个文件夹——运行过的文件夹含 `node_modules`、`.local-data`（数据库 + 你的浏览器登录态）等，体积近 300MB，而且会把你的登录会话泄露出去。干净包解压后双击启动脚本即可，依赖自动安装。
